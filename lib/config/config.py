...
tf.app.flags.DEFINE_integer('snapshot_iterations', 5000, "Iteration to take snapshot")      #保存模型的频率，可以改这个进行测试，网络是否可行


FLAGS2["scales"] = (600,)
FLAGS2["test_scales"] = (600,)
FLAGS2['timeF'] = 1
FLAGS2['process_speed'] = 0.1       #几秒一张图,只是实时处理视频的地方用得到
######################
# Testing Parameters #
######################
...
tf.app.flags.DEFINE_integer('rpn_top_n', 100, "Only useful when TEST.MODE is 'top', specifies the number of top proposals to select")
...
# Proposal Parameters #
#######################
tf.app.flags.DEFINE_float('proposal_fg_fraction', 0.4, "Fraction of minibatch that is labeled foreground (i.e. class > 0)")
...
tf.app.flags.DEFINE_float('roi_bg_threshold_low', 0.0, "Overlap threshold for a ROI to be considered background (class = 0 if overlap in [LO, HI))")
...
FLAGS2['temp_dir'] = osp.abspath(osp.join(FLAGS2["root_dir"],'data\VOCDevkit2007\VOC2007\JPEGImages'))
# FLAGS2['temp_dir'] = 'E:\FasterRcnn\Faster-RCNN-TensorFlow-Python3.5-master\data\VOCDevkit2007\VOC2007\JPEGImages'
FLAGS2['num_annotations'] = 2000
FLAGS2['cur_place'] = ''
FLAGS2['address'] = ('192.168.137.198',10086)
FLAGS2['process_json_info_rate'] = 60
FLAGS2['video_name'] = '02.mp4'
