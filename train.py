...
class Train:
    ...
    def train(self):
        ...
        def get_session(gpu_fraction=0.8):
            '''Assume that you have 6GB of GPU memory and want to allocate ~4GB'''

            num_threads = os.environ.get('OMP_NUM_THREADS')
            gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=gpu_fraction)
            if num_threads:
                print('new threads are allocated ... ')
                config = tf.ConfigProto(
                    allow_soft_placement=True, gpu_options=gpu_options, intra_op_parallelism_threads=num_threads)
        ...
        restorer.restore(sess, cfg.FLAGS.pretrained_model)  #加载数据模型
        print('Loaded.')
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            if blobs==None:
                iter += 1
                if iter % cfg.FLAGS.snapshot_iterations == 0:  # 保存的频率
                    self.snapshot(sess, iter)
                continue
            if blobs['gt_boxes'][0][1] > blobs['gt_boxes'][0][3] or blobs['gt_boxes'][0][0] > blobs['gt_boxes'][0][2]:
                iter += 1
                if iter % cfg.FLAGS.snapshot_iterations == 0:  # 保存的频率
                    self.snapshot(sess, iter)
                continue
