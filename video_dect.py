...
# 日期  2019.4.28
# 修改者： 李博深
# 修改目标： 通过修改输出视频的fps，满足视频处理时间与原视频相同。
# 可以根据用户传入的视频，结合网络速度，算出输出视频的FPS，从而保证处理时间与原视频时间大致相同
# 视频---->已标注的图片
#
...
def split_video_to_image(video='', video_name='', jpg_name=''):
    start_time = time.time()
    step = 0
    vc = cv2.VideoCapture(video)
    FPS = vc.get(cv2.CAP_PROP_FPS)
    speed = cfg.FLAGS2['process_speed']
    TimeF = FPS * speed
    ...
    return TimeF,width,height,int(FPS/TimeF)
...
if __name__ == '__main__':
    ...
    video_name = cfg.FLAGS2['video_name']
    ...
    TimeF,width,height,cur_fps = split_video_to_image(video_path, video_name, jpg_name=img_path) # 视频--->图片（存入demo/temp_jpg中）
    ...
    if width==-1 or height==-1:
        ...
        for i_name in im_names:
            os.remove(os.path.join(img_path,i_name))#视频合并完成后应该删除截取的图片


        # 20119.5.8修改，目标，为了让处理视频的时间跟视频时间差不多，需要修改待输出的视频的fps
        print("视频合并中...")
        from my_utils.video_comb import *
        save_vid_name = video_name
        if con_im_to_video(timeF=TimeF,fps=int(cur_fps), size=size,name=save_vid_name):
            print('视频 {} 合成成功...'.format(cfg.FLAGS2['data_dir']+'\save_videos/'+save_vid_name))
        print('fps = ',cur_fps)
        print('**'*50)
        people_info = process_json_info(cur_fps)
        people_info = json.dumps(people_info)
        with open(os.path.join(cfg.FLAGS2['data_dir'],'save_videos',video_name.split('.')[0]+'.json'),'w') as f:
            f.write(people_info)
        print('people info ',people_info)
        print('has saved at ',cfg.FLAGS2['data_dir']+'/save_videos/'+video_name.split('.')[0]+'.json')
